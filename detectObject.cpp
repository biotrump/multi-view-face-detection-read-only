...
// Can use Haar cascades or LBP cascades for Face Detection, or even eye, mouth, or car detection.
// Input is temporarily shrunk to 'scaledWidth' for much faster detection, since 200 is enough to find faces.
void detectObjectsCustom_afterprocess(const Mat &equalizedImg, double scale, int cols, int rows, CascadeClassifier &cascade, vector<Rect> &objects, int scaledWidth, int flags, Size minFeatureSize, float searchScaleFactor, int minNeighbors)
{
    // Detect objects in the small grayscale image.
    cascade.detectMultiScale(equalizedImg, objects, searchScaleFactor, minNeighbors, flags, minFeatureSize);
    // Enlarge the results if the image was temporarily shrunk before detection.
    ...
    for (int i = 0; i < (int)objects.size(); i++ ) {
        ...
        if (objects[i].y < 0)
            objects[i].y = 0;
        if (objects[i].x + objects[i].width > cols)
            objects[i].x = cols - objects[i].width;
        if (objects[i].y + objects[i].height > rows)
            objects[i].y = rows - objects[i].height;
    }
    // Return with the detected face rectangles stored in "objects".
}
...
void detectLargestObject_afterprocess(const Mat &equalizedImg, double scale, int cols, int rows, CascadeClassifier &cascade, Rect &largestObject, int scaledWidth)
{
    // Only search for just 1 object (the biggest in the image).
    int flags = CASCADE_FIND_BIGGEST_OBJECT;// | CASCADE_DO_ROUGH_SEARCH;
    // Smallest object size.
    ...
    // Perform Object or Face Detection, looking for just 1 object (the biggest in the image).
    vector<Rect> objects;
    detectObjectsCustom_afterprocess(equalizedImg, scale, cols, rows, cascade, objects, scaledWidth, flags, minFeatureSize, searchScaleFactor, minNeighbors);
    if (objects.size() > 0) {
        // Return the only detected object.
        largestObject = (Rect)objects.at(0);
    }
